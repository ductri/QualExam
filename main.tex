...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item The optimal solution of this problem has a particular structure, which is:
            \begin{itemize}
                \item A part of $\bm{C}_{\text{opt}}$ where row indices in $\mathcal{K}$ are $\bm{H}$
                \item The other are $ \bm{0}$ rows.
            \end{itemize}
...
\begin{frame}
    ...
    \end{alignat*}
     \begin{itemize}
         \item The greedy approach identifies set $\mathcal{K}$ by adding one index at a time \citep{fu2014self}.
...
% For a noiseless case,
  ...
  \note{
      \begin{itemize}
          ...
          \item As example a solution $\bm{C}^{\star}$. It is a desired solution because by inspecting $\bm{C}^{\star}$, we can extract $\mathcal{K}$ exactly.
          ...
          \item Firstly, both methods converge and gives a good solutions in terms of objective value. However, if we look at non zero row in the optimal solution, only FW reveals perfect $\mathcal{K}$. 
          
          \item Secondly, if we take a look at the density of $\bm{C}$ during the optimization produce, we can see that FW consistently keeps $\bm{C}$ being very sparse, compared to APG. This is the key for memory efficiency when using FW.
      \end{itemize}
  }
\end{frame}
...
% \item With an initialization $\bm{x}^{0} = \bm{0}$, Can we show $n^{\star} \in \mathcal{K}$?
    ...
    \begin{itemize}
        ...
        In detail, the solution $\bm{s}$ for this step is a canonical unit vector, where the index $n^{\star}$ is index of the smallest element of the gradient.
    \end{itemize}
    }
\end{frame}
...
\begin{frame}[label=fine]
    ...
    \note{
    \begin{itemize}
        \item Get back to our problem, note that the original problem is decomposible and hence we can solve it for each column of $\bm{C}$ independently.
...
\item We propose the following formulation
    ...
    \begin{itemize}
        ...
        \item We will refer our FW implement on this problem as MERIT
    \end{itemize}
    }
\end{frame}
