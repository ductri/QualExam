...
\tikzset{%
  ...
  myhighlight/.style={rectangle,fill=blue,draw=none,fill opacity=.1,inner sep=0pt}
}
...
\note{
\begin{itemize}
    \item So we will need to restrict our model to by imposing some assumption to gain identifiability. There are several conditions that can guarantee identifiability. One of the well-known conditions is separability condition.
    \item It states that \ldots 
    \item This condition has been exploited in terms of algorithm design. Particularly, the problem of finding $\bm{W}, \bm{H}$ boils down to finding the set $\mathcal{K}$. The rationale is like this.
...
\begin{frame}
    ...
    \note{
    \begin{itemize}
        \item There have been many formulations developed for problem of finding $\mathcal{K}$ under separability condition.
            One interesting perspective is from dictionary learning. The objective function is a row-0 norm, which counts number of nonzero in $\bm{C}$.
        \item {\blue [Look at figure]} Under this formulation, we are looking for a smallest subset of columns for $\bm{X}$ to present itself.
        \item To see how does this formulation help revealing $\mathcal{K}$, we will take a look at the optimal solution. 
        \item Under noiseless case, the solution can be constructed  as follows. The rows corresponding to $\mathcal{K}$ is the ground truth $\bm{H}$, and the other rows are $ \bm{0}$.
...
\begin{frame}
    ...
    \end{alignat*}
     \begin{itemize}
         \item The greedy approach identifies set $\mathcal{K}$ one index by a time by looking at columns in $\bm{X}$ \citep{fu2014self}.
         \item Successive projection algorithm \citep{MC01} is a representative. 
         \item Extracting $\mathcal{K}$ is even guaranteed in noisy case \citep{gillis2014fast}.
         \item All greedy-based methods have a Gram-Schmidt structure which is prone to error propagation under noisy condition.
     \end{itemize}
     \note{
\begin{itemize}
    \item Solving this optimization problem is not a trivial task. It is firstly not a convex problem due to the row-0, and secondly, and more importantly, it is a combinatorial problem in essence. So a native greedy search would be prohibited even if number of vertices $K$ is given.
        ...
        \item Methods under this approach has a Gram-Schidt-like structure in their algorithms. Since it addes an index one by one, when noise is present, an error from one step will be accumulated and effect future steps. This make methods under this approach sensitive to noise.
\end{itemize}
     }
\end{frame}
...
\item Often more robust than greedy approach
    ...
    \begin{itemize}
        ...
        \item As mentioned before, the original problem is non-convex, which makes it hard in terms of optimisation. 
        \item So one way is to use a convex opt problem as a surrogate. There has been many convex relaxation formulations proposed in the literature. 
            % One example is this formulation where the row-0 is relaxed by the so-called mixed norm. 
        ...
        \item We aim to have an algorithm that can leverage the convex relaxation approach because of its noise robustness, and also be able to work on large scale problem.
    \item Toward this end, we propose using Frank-Wolfe method to achieve that goal.
    \end{itemize}
    }
\end{frame}
...
% For a noiseless case,
  ...
  \note{
      \begin{itemize}
          \item To see how FW can realize our goal, let's start with a noiseless case. Consider a simple optimisation problem. This problem is convex, but could have multiple solution.
          ...
          \item To demonstrate FW's magic, we compare results when running Accelerated Proximal Gradient, a typical first order method for constrained optimisation problem, and compare it with Frank-Wolfe.
          \item As we can see, both methods converged and gives a good solutions in terms of objective value. However, there are 2 interesting and important points. Firstly, only FW outputs a solution $\bm{C}^{\star}$ which is perfectly reveal $\mathcal{K}$.
          \item Secondly, if we take a look at the density of $\bm{C}$ against iterations, we can see that FW consistently keeps $\bm{C}$ being very sparse, compared to APG. This is the key for memory efficiency when using FW.
      \end{itemize}
  }
\end{frame}
...
% \item With an initialization $\bm{x}^{0} = \bm{0}$, Can we show $n^{\star} \in \mathcal{K}$?
    ...
    \begin{itemize}
        ...
        \item It solves for convex obj \ldots 
        \item A standard update procedure is 
        \item In our case, the constraint is \ldots 
    \end{itemize}
    }
\end{frame}
...
% \begin{block}
%     
% FW's updating procedure can be performed using $O(N)$ memory.
% \end{block}
% \begin{itemize}
%     \item Recall the update steps are
%         \begin{align}
%         &\bm{S}^{t} \leftarrow \argmin_{\bm{S} \in \mathcal{D}} \; \langle \underbrace{\nabla f(\bm{C})}_{\bm{G}}, \bm{S} \rangle  \label{eq:sub_S} \\
%         &\bm{C}^{t+1} \leftarrow \bm{C}^{t} + (1-\alpha) (\bm{S}^{t}-\bm{C}^{t}), \quad \alpha^{t} = 2/(2+t)
%         \end{align} 
%     \item This procedure can be performed on columns of $\bm{C}$ individually
%     % \begin{itemize}
%     %     \item Simplex constraint is columns-wise imposed.
%     %     \item Linear objective function can be decomposed to columns of $\bm{C}$.
%     % \end{itemize}
%     \begin{align}
%     &\bm{s}_\ell^{t} \leftarrow \argmin_{\bm{s}_\ell \geq 0, \bm{1}^{\T}\bm{s}=1} \; \langle \bm{g}_\ell, \bm{s}_\ell \rangle  \\
%     &\bm{c}_\ell^{t+1} \leftarrow \bm{c}_\ell^{t} + (1-\alpha) (\bm{s}_{\ell}^{t}-\bm{c}_{\ell}^{t}), \quad \alpha^{t} = 2/(2+t)
%     \end{align} 
%     \item Moreover, 
% $ \bm{g}_\ell = \bm{X}^{\T}(\bm{X}\bm{c}_\ell - \bm{x}_\ell) + \lambda [\nabla \varPhi_{\mu}(\bm{C})]_{:, \ell} $ where
% \[
%     [\nabla \varPhi_{\mu}(\bm{C})]_{n,\ell} = \dfrac{\exp (c_{n, \ell}/\mu)}{\sum^{N}_{i=1} \exp(c_{n, i}/\mu)} 
% \] 
% \end{itemize}
...
%     If 
%     \begin{itemize}
%         \item $\text{supp}(\bm{c}_\ell^{t}) \subseteq \mathcal{K}$ for all $\ell$.
%         \item There exists $k \in \mathcal{K}$, such that $\bm{C}^{t}(k, :) \neq t \bm{1}^{\T}$.
%         \item Noise is bounded as $ \max_{n} \norm{\bm{v}_n}_2 \leq  \sqrt{\gamma^2 + \upsilon } -\gamma $ 
% where 
% \begin{align*}
% &\upsilon= \lambda (1/N - \exp(\mu/\psi))/4 - D , \quad \quad
...
%     \end{itemize}
%     then FW will update an $j_{\ell}$-th element of  $\bm{c}_\ell^{t}$ such that $j_\ell \in \mathcal{K}$ for all $\ell$.
%     % $\text{supp}(\bm{c}_{\ell}^{t+1}) \subseteq \mathcal{K}$ for all $\ell$.
%
% \end{theorem}
%
% % Similar to the noiseless case, FW can always pick an index $n \in \mathcal{K}$ to update $\bm{c}_\ell$ with the help of regularization.
% % \[ 
% %     \bm{g}_\ell = \underbrace{\bm{X}^{\T}(\bm{X}\bm{c}_\ell - \bm{x}_\ell)}_{\bm{q}_\ell} + \lambda \underbrace{[\nabla \varPhi_{\mu}(\bm{C})]_{:, \ell}}_{\bm{y}_\ell} , \quad 
% %     y_{n,\ell} = \dfrac{\exp (c_{n, \ell}/\mu)}{\sum^{N}_{i=1} \exp(c_{n, i}/\mu)} 
% % \]
% % \begin{itemize}
% %     \item For $k \notin \mathcal{K}$,
% %         $ [\nabla \varPhi_{\mu}(\bm{C})]_{k, \ell} = 1/N$. 
% %     % \item $\exists k \in \mathcal{K}, \bm{C}(k, :)$ is not a constant $\Rightarrow \exists k \in \mathcal{K} y_{k, \ell}$ is not the largest element in row $k$-th.
% %     \item If $ \exists k\in \mathcal{K}$ such that $c_{k, \ell}$ is not the largest element in row $k$-th (*), then
% %          % $ y_{k, l} \approx 0 $  for certain $\mu$.
% %          $
% %          y_{k, \ell} < \exp ((c_{k, \ell} - c_{k, \star})/\mu), \quad c_{k, \star} = \max_{i} c_{k, i}
% %          $ 
% %     \item Therefore, 
% %         \[
% %             \begin{cases}
% %                 y_{k, \ell} \approx 0 \quad & \text{if } k\in \mathcal{K}\\
% %                 y_{k, \ell} = \lambda/N \quad & \text{if } k \notin \mathcal{K}
% %             \end{cases}
% %         \] 
% %     \item (*) always holds if there exists $k \in \mathcal{K}$ such that $\bm{C}(k, :)$ is not a constant
% % \end{itemize}
%
%
% \note{
...
\begin{frame}[label=current]
    ...
    \end{block}
% An  example of $\bm{C}$,
% Gradient respect to column $\bm{c}_{\ell}$,
\begin{align*} 
